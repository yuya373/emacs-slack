...
(defun slack-add-code-overlay ()
  (let ((regex "^[[:blank:]]*\\(```\\)\\(\\s *?\\)$")
        (regex-begin "^[[:blank:]]*\\(```\\)\\([[:blank:]]*{?[[:blank:]]*\\)\\([^[:space:]]+?\\)?\\(?:[[:blank:]]+\\(.+?\\)\\)?\\([[:blank:]]*}?[[:blank:]]*\\)$"))
    (while (re-search-forward regex nil t)
      (let ((cur-point (point))
            (end (match-end 2)))
        (message "CUR_POINT: %s, END: %s, %s" cur-point end (buffer-substring-no-properties (point-min) cur-point))
        (goto-char (match-beginning 1))
        (when (re-search-backward regex-begin nil t)
          (let ((beg (match-beginning 1)))
            (when (and beg end)
              (message "BEG: %s, END: %s, %s" beg end (buffer-substring-no-properties beg end))
              (slack-put-preview-overlay beg end)))
          (goto-char cur-point))
        )
      )
    ))
